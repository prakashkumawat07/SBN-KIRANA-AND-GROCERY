...
export default function ProductDetail(){
  ...
  function buyNow(){if(!productAvailable(product))return;addToCart(product);nav('/checkout')}
  ...
  const save=Math.max((product.mrp||0)-(product.price||0),0);const canReview=Boolean(user&&eligibility?.eligible);const hasPublicReviews=Number(reviews.count)>0;const existing=eligibility?.existing;const marketing=MARKETING_BADGES[product.customerBadge]||null;const available=productAvailable(product);

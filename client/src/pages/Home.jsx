...
export default function Home(){
  const [products,setProducts]=useState(fallbackProducts.filter(p=>p.featured));
  const [offers,setOffers]=useState([]);
  useEffect(()=>{api('/products?featured=true').then(data=>{if(Array.isArray(data)&&data.length)setProducts(data)}).catch(()=>{});api('/offers').then(d=>setOffers(Array.isArray(d)?d:[])).catch(()=>{})},[]);
  const featuredOffer=offers.find(o=>o.featured)||offers[0];
  return <main className="market-home">
    <section className="market-hero">
      <div className="hero-main-card"><div className="hero-copy-new"><span className="hero-kicker">EVERYDAY VALUE • LOCAL SPEED</span><h1>Everything your home needs, <em>one smart basket away.</em></h1><p>Shop fresh groceries, pantry essentials, snacks and home care with quick local fulfilment.</p><div className="hero-buttons"><Link className="shop-now" to="/products">Shop now →</Link><Link className="outline-btn" to="/bulk-orders">Request bulk quote</Link></div><div className="trust-row"><span>✓ Fresh picks</span><span>✓ Secure account</span><span>✓ Easy support</span></div></div><div className="hero-art"><div className="basket-visual"><span>🥦</span><span>🍊</span><span>🥖</span><span>🥛</span><span>🍅</span></div><div className="price-pop"><small>FREE DELIVERY</small><b>Orders ₹499+</b></div></div></div>
      <div className="hero-side"><Link to="/paylater" className="side-card credit-side"><span>₹</span><div><small>SBN PAYLATER</small><h3>Buy essentials on approved store credit</h3><p>Manual approval • Clear limit • Track dues</p></div></Link><Link to="/bulk-orders" className="side-card deal-side"><span>📋</span><div><small>BULK ORDERS</small><h3>Upload a list and get a quotation</h3><p>Offices • Events • Retail • Large family orders</p></div></Link></div>
    </section>

    {featuredOffer&&<section className="live-offer-banner"><div><span>LIVE OFFER</span><h2>{featuredOffer.title}</h2><p>{featuredOffer.description||`Use code ${featuredOffer.code} at checkout.`}</p></div><div className="live-offer-code"><small>COUPON CODE</small><b>{featuredOffer.code}</b><span>{featuredOffer.type==='percent'?`${featuredOffer.value}% OFF`:`₹${featuredOffer.value} OFF`}{featuredOffer.minOrder?` · Min ₹${featuredOffer.minOrder}`:''}</span></div><Link to="/products">Shop offer →</Link></section>}

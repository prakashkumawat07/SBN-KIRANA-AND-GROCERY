...
const CONTEXT='sbn-kirana-paylater-payment-v1';
function key(){
  const source=process.env.PAYMENT_PROOF_ENCRYPTION_KEY||process.env.KYC_ENCRYPTION_KEY||process.env.JWT_SECRET||'';
  if(!source)throw new Error('Payment proof encryption secret is not configured');
  return crypto.createHash('sha256').update(`${CONTEXT}:${source}`).digest();
}
export function encryptPaymentProof(value){
...
export function decryptPaymentProof(payload={}){
